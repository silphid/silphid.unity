...
namespace Silphid.Extensions
{
    public abstract class VelocityObservableBase<T> : OperatorObservableBase<T>
    {
        ...
        protected VelocityObservableBase(IObservable<T> source, float smoothness, IObservable<T> velocities, Func<float> getTime)
        ...
        protected override IDisposable SubscribeCore(IObserver<T> observer, IDisposable cancel) =>
            new VelocityObserver(_source, _velocities, _getTime, GetVelocity, GetSmoothed, _smoothness, observer, cancel).Run();
        ...
        private class VelocityObserver : OperatorObserverBase<T, T>
        {
            ...
            public VelocityObserver(IObservable<T> source, IObservable<T> velocities, Func<float> getTime, Func<T, T, float, T> getVelocity, Func<T, T, float, T> getSmoothed, float smoothness, IObserver<T> observer, IDisposable cancel) : base(observer, cancel)
            {
                _source = source;
                _velocities = velocities;
