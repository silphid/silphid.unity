...
namespace Silphid.Loadzup.Bundles
{
    ...
    internal class LoadingBundle : IBundle
    {
        ...
        public IObservable<T> LoadAsset<T>(string assetName)
        {
            return (_isDisposed
                        ? Observable.Throw<T>(
                            new InvalidOperationException("The bundle is disposed. Cannot LoadAsset from it"))
                        : _bundle.LoadAsset<T>(assetName)).Finally(Dispose);
        }
        ...
        public IObservable<T[]> LoadAllAssets<T>()
        {
            return (_isDisposed
                        ...
                        : _bundle.LoadAllAssets<T>()).Finally(Dispose);
        }
