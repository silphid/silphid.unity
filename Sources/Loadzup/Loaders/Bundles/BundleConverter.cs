...
namespace Silphid.Loadzup.Bundles
{
    public class BundleConverter : ConverterBase<byte[]>
    {
        ...
        protected override bool SupportsInternal<T>(byte[] input, string mediaType) =>
            typeof(T) == typeof(IBundle);

        protected override IObservable<object> ConvertAsync<T>(byte[] input, string mediaType, Encoding encoding) =>
            AssetBundle.LoadFromMemoryAsync(input)
