...
namespace Silphid.Showzup
{
    public class ScoreEvaluator : IScoreEvaluator
    {
        ...
        public const int InterfacePenality = 2;
        ...
        {
            const int score = TypeScore - InterfacePenality;

            if (!requestedType.GetInterfaces().Contains(candidateInterface))
                return null;

            var type = requestedType.BaseType;
            var inheritanceDepth = 0;

            while (type != null)
            {
                if (type.GetInterfaces().Contains(candidateInterface))
                    inheritanceDepth++;

                type = type.BaseType;
            }

            return score - InheritanceDepthPenality * inheritanceDepth;
        }
    }
}