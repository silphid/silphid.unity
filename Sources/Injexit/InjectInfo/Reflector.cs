    ...
    public class Reflector : IReflector
    {
        ...
        public Reflector()
        {
            _gameObjectTypeInfo = new InjectTypeInfo(
                typeof(GameObject),
                new InjectConstructorInfo(
                    null,
                    new InvalidOperationException("GameObject cannot be instantiated manually."),
                    null),
        ...
        private ConstructorInfo ResolveConstructor(Type type)
        {
            ...
            constructors = constructors.Where(x => x.HasAttribute<InjectAttribute>())
                                       .ToArray();
            if (constructors.Length == 1)
                return constructors.First();
        ...
        private InjectParameterInfo[] GetParameters(MethodBase method) =>
            (from parameter in method.GetParameters()
             ...
             select new InjectParameterInfo(parameter, isOptional)).ToArray();
            ...
            foreach (var property in properties)
            {
                ...
                if (attribute != null)
                {
                    if (!property.CanWrite)
                        throw new InvalidOperationException(
                            $"Property {type.Name}.{property.Name} is marked with [Inject] attribute, but has not setter.");
